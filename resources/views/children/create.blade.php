...
@section('title', 'Tambah Pasien - SI-MOKA')
...
<div class="max-w-3xl mx-auto">
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-slate-500 text-sm">Hubungkan pasien dengan orang tua dan terapis.</p>
        </div>
    </div>
    ...
    <form action="{{ route('children.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-4 mt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="jenis_kelamin" value="L" class="w-4 h-4 text-blue-600 border-slate-300 focus:ring-blue-500" {{ old('jenis_kelamin') == 'L' ? 'checked' : '' }} required>
                            <span class="text-sm text-slate-600 font-medium">Laki-laki</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="jenis_kelamin" value="P" class="w-4 h-4 text-pink-600 border-slate-300 focus:ring-pink-500" {{ old('jenis_kelamin') == 'P' ? 'checked' : '' }}>
                ...
                <div class="space-y-2">
                    ...
                    <select name="therapis_id" id="therapis_id"
                        ...
                        @foreach($therapists as $therapist)
                            <option value="{{ $therapist->id }}" {{ old('therapis_id') == $therapist->id ? 'selected' : '' }}>
                                {{ $therapist->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <!-- Catatan Medis -->
            <div class="space-y-2">
                <label for="catatan_medis" class="text-sm font-bold text-slate-700">Catatan Medis Awal</label>
                <textarea name="catatan_medis" id="catatan_medis" rows="4"
                          ...
                          placeholder="Keluhan utama, diagnosis medis, atau catatan sensorik awal...">{{ old('catatan_medis') }}</textarea>
