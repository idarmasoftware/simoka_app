<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 {{ request()->routeIs('dashboard') ? 'bg-blue-500 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-slate-50 transition' }} px-4 py-3 rounded-xl font-medium">
            ...
            {{ auth()->user()->isTerapis() ? 'Dashboard Terapis' : (auth()->user()->isOrangTua() ? 'Dashboard Orang Tua' : 'Dashboard') }}
        </a>

        @if(auth()->user()->isTerapis() || auth()->user()->isSuperAdmin())
            <a href="{{ route('assessments.select_child') }}" class="flex items-center gap-3 {{ request()->routeIs('assessments.select_child') || request()->routeIs('assessments.create') ? 'bg-blue-500 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-slate-50 transition' }} px-4 py-3 rounded-xl font-medium">
                <svg class="w-5 h-5 {{ request()->routeIs('assessments.select_child') || request()->routeIs('assessments.create') ? 'text-white' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
                Assessment SSP
            </a>
        @endif

        <a href="{{ route('assessments.index') }}" class="flex items-center gap-3 {{ request()->routeIs('assessments.index') || request()->routeIs('assessments.show') ? 'bg-blue-500 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-slate-50 transition' }} px-4 py-3 rounded-xl font-medium">
            <svg class="w-5 h-5 {{ request()->routeIs('assessments.index') || request()->routeIs('assessments.show') ? 'text-white' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
            Riwayat Assessment
        </a>

        <a href="{{ route('assessments.progress') }}" class="flex items-center gap-3 {{ request()->routeIs('assessments.progress') ? 'bg-blue-500 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-slate-50 transition' }} px-4 py-3 rounded-xl font-medium">
            <svg class="w-5 h-5 {{ request()->routeIs('assessments.progress') ? 'text-white' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"></path></svg>
            Progres Perkembangan
        </a>

        <a href="{{ route('tasks.index') }}" class="flex items-center gap-3 {{ request()->routeIs('tasks.*') ? 'bg-blue-500 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-slate-50 transition' }} px-4 py-3 rounded-xl font-medium">
            <svg class="w-5 h-5 {{ request()->routeIs('tasks.*') ? 'text-white' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
            {{ auth()->user()->isTerapis() ? 'Kelola Tugas Rumah' : 'Tugas Rumah Anak' }}
        </a>
        ...
        <a href="{{ route('children.index') }}" class="flex items-center gap-3 {{ request()->routeIs('children.*') ? 'bg-blue-500 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-slate-50 transition' }} px-4 py-3 rounded-xl font-medium">
            ...
            {{ auth()->user()->isOrangTua() ? 'Kelola Anak' : 'Kelola Pasien' }}
        </a>

        @if(auth()->user()->isSuperAdmin() || auth()->user()->isTerapis())
